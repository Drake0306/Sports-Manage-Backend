...
const joinTeam = async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ error: true, message: "Token missing from header" });
    }
    ...
    if (!teamCode) {
      return res
        .status(400)
        .json({ error: true, message: "Team code is required" });
    }
    ...
    if (existingEntry) {
      ...
      return res.status(200).json({
        error: false,
        message:
          "Successfully updated team entry to active, other entries are now inactive",
        data: existingEntry,
      });
    }
    ...
    if (differentTeamEntry) {
      ...
      return res.status(201).json({
        error: false,
        message:
          "User was already in a different team; joined new team successfully",
        data: newJoinedTeam,
      });
    }
...
const fetchEvent = async (req, res) => {
  try {
    ...
    const events = await EVENTS.findAll({
      where: {
        coachId: decodedToken.id, // Filter by coachId
      },
    });
...
const changePassword = async (req, res) => {
  ...
  if (!authHeader || !oldPassword || !newPassword) {
    return res.status(400).json({
      error: true,
      message:
        "Authorization token, old password, and new password are required",
    });
  }
